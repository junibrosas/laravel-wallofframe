        ...
        <div class="row">
            <div class="col-md-10">
                <div class="frame-preview">
                    <img ng-src="{{ /*$product->present()->imageCachePreview*/ url('images/frame-size/'.$product->image.'?') }}@{{ 'width=' +(currentSize.width+60) +'&height='+(currentSize.height+60) }}" data-zoom-image="{{ $product->present()->imageCachePreview }}" alt="Original Image" class="frame-preview-image img-responsive elem-center custom-border" style="@{{ currentFrame.borderStyle }} border-image-source: url('@{{ currentFrame.imagePath }}'); width: @{{ (currentSize.width+60) }}};" />
                </div>
            </div>
            <div class="col-md-2">
